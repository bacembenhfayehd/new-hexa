...
export const AdminContextProvider = (props) => {
  ...
  const createProduct = async (productData, imageFiles) => {
    ...
    try {
      const response = await fetch("https://api.hexagrow-indus.com/api/admin/products", {
        method: "POST",
        body: formData,
      });
 ...
 const updateProduct = async (productId, updateData) => {
  try {
    ...
    const response = await fetch(`https://api.hexagrow-indus.com/api/admin/products/${productId}`, {
      method: 'PUT',
  ...
  const getAllOrders = async (page = 1) => {
    try {
      const response = await fetch(
        `https://api.hexagrow-indus.com/api/admin/orders?page=${page}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
   ...
   const fetchUsers = async (page = 1, limit = 10, search = "", sortBy = "createdAt", sortOrder = "desc") => {
    try {
      setLoading(true);
      const res = await axios.get("https://api.hexagrow-indus.com/api/admin/users", {
  ...
  const fetchComments = async ({ page = 1, limit = 10, search = "", sortBy = "createdAt", sortOrder = "desc" } = {}) => {
    try {
      setLoading(true);
      const res = await axios.get("https://api.hexagrow-indus.com/api/admin/", {
        params: { page, limit, search, sortBy, sortOrder }
      });
  ...
  try {
    const response = await fetch(`https://api.hexagrow-indus.com/api/admin/comments/${commentId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
 ...
 const deleteUser = async (userId) => {
  try {
    ...
    const response = await fetch(`https://api.hexagrow-indus.com/api/admin/usersdelete/${userId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      }
...
const updateOrderStatus = async (orderId, newStatus) => {
  try {
    ...
    const response = await fetch(`https://api.hexagrow-indus.com/api/admin/orders/${orderId}/status`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
...
const exportUsers = async () => {
  try {
    ...
    const response = await axios.get('https://api.hexagrow-indus.com/api/admin/users/export', {
